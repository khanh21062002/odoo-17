...
class SimTelecom(http.Controller):
    @http.route('/api/sim_telecom/get_sim_telecom',
                methods=['POST'],
                type='http',
                auth='none',
                cors='*',
    ...
    def get_sim_telecom(self, **kw):
        try:
            domain = []
            sim_telecom = request.env['dth.kho.telecom.supplier'].sudo().search_read(domain, fields=['code', 'name', 'sequence'], order='sequence asc')
            result = {
                "data": sim_telecom
            }
